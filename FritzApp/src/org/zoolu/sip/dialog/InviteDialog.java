...
public class InviteDialog extends Dialog implements TransactionClientListener,
	...
	/** fix/hack: last invite message as class variable necessary for missing ACKs 4xx responses. */
	protected static Message last_invite_req;
		...
		if (tc.getTransactionMethod().equals(SipMethods.INVITE)) {
			...
			if (statusIs(D_ReINVITING)) {
				...
				if (code >= 300 && code < 400)
					...
					// Fix: ACK is needed after reception of 487 - this fixes endless
					// ringing on remote side after cancelling an outgoing call. NB
					this.sendAckOnTransFailureResponse(msg); 
	...
	/** Fix for missing ACK after 4xx responses in INVITE transaction. */
	private void sendAckOnTransFailureResponse(Message resp) {
		if(last_invite_req != null) {
			Message ack = MessageFactory.createNon2xxAckRequest(sip_provider, last_invite_req, resp);
