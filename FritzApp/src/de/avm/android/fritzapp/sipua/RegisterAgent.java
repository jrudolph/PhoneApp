...
public class RegisterAgent implements TransactionClientListener, SubscriberDialogListener {
			...
			Message resp) 
	{
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			...
			{
				String contact_string_local =   "<sip:" + user_profile.contact_url + ">";
				Vector<Header> contacts = resp.getContacts().getHeaders();
				for (int i = 0; i < contacts.size(); i++) {
					String contact_string  = (new ContactHeader((Header) contacts.elementAt(i))).getValue();
					// compare that the contact_url is our contact_url
					if( contact_string.contains(contact_string_local) ) {
